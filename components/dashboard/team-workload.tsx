      ...
      <CardContent className="space-y-4 min-w-0">
        ...
        {team.slice(0, 6).map((m) => (
          <div key={m.id} className="flex items-center gap-3 min-w-0">
            <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-indigo-500 to-indigo-700 text-xs font-bold text-white">
              {initials(m.name)}
            </div>
            ...
            <div className="min-w-0 flex-1">
              <div className="flex items-center justify-between gap-2">
                ...
                <span className="font-num shrink-0 text-xs font-semibold text-slate-600">
                  {m.workload}%
                </span>
              </div>
